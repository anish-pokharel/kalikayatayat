...
//   // Get fare for bus type and route
//   getBusFare(routeId: string, busType: string): Observable<ApiResponse<FareInfo>> {
//     return this.http.get<ApiResponse<FareInfo>>(`${this.apiUrl}/fare/calculate?routeId=${routeId}&busType=${busType}`);
//   }
...
export interface BusSchedule {
  ...
  totalSeats: number;
  availableSeats: number;
  status: 'scheduled' | 'cancelled' | 'completed' | 'delayed';
  busNumber?: string;
  busName?: string;
  busType?: string;
  routeName?: string;
...
export class BusService {
  ...
  getBuses(filters?: any): Observable<ApiResponse<Bus[]>> {
    ...
    if (filters) {
      ...
      if (filters.limit) params.push(`limit=${filters.limit}`);
      // Institute filters
      if (filters.instituteId && filters.instituteId !== 'all') params.push(`instituteId=${filters.instituteId}`);
      if (filters.instituteName && filters.instituteName !== 'all') params.push(`instituteName=${encodeURIComponent(filters.instituteName)}`);
    }
  ...
  // Get buses by institute
  getBusesByInstitute(filters?: any): Observable<ApiResponse<Bus[]>> {
    let url = `${this.apiUrl}/admin/buses/by-institute`;
    const params: string[] = [];
  ...
  // Get buses for current user's institute
  getMyInstituteBuses(filters?: any): Observable<ApiResponse<Bus[]>> {
    let url = `${this.apiUrl}/admin/buses/my-institute`;
    const params: string[] = [];
  ...
  // Create bus with institute information
  createBusWithInstitute(busData: Partial<Bus>): Observable<ApiResponse<Bus>> {
    return this.http.post<ApiResponse<Bus>>(`${this.apiUrl}/admin/buses/with-institute`, busData);
  }
  ...
  // Update bus with institute information
  updateBusWithInstitute(id: string, busData: Partial<Bus>): Observable<ApiResponse<Bus>> {
    return this.http.put<ApiResponse<Bus>>(`${this.apiUrl}/admin/buses/with-institute/${id}`, busData);
  }
  ...
  // Get institute bus statistics
  getInstituteBusStats(): Observable<ApiResponse<any>> {
    return this.http.get<ApiResponse<any>>(`${this.apiUrl}/admin/institute/bus-stats`);
  }

  // Get buses by institute name
  getBusesByInstituteName(instituteName: string, filters?: any): Observable<ApiResponse<Bus[]>> {
    let url = `${this.apiUrl}/admin/buses/institute/${encodeURIComponent(instituteName)}`;
    const params: string[] = [];
  ...
  createSchedule(scheduleData: any): Observable<ApiResponse<BusSchedule>> {
    return this.http.post<ApiResponse<BusSchedule>>(`${this.apiUrl}/admin/schedules`, scheduleData);
  }

  // Get all schedules with filters
  getSchedules(filters?: any): Observable<ApiResponse<BusSchedule[]>> {
    ...
    return this.http.get<ApiResponse<BusSchedule[]>>(url);
  }

  // Get a single schedule by ID
  getScheduleById(id: string): Observable<ApiResponse<BusSchedule>> {
    return this.http.get<ApiResponse<BusSchedule>>(`${this.apiUrl}/admin/schedules/${id}`);
  }

  // Update a schedule
  updateSchedule(id: string, scheduleData: any): Observable<ApiResponse<BusSchedule>> {
    return this.http.put<ApiResponse<BusSchedule>>(`${this.apiUrl}/admin/schedules/${id}`, scheduleData);
  }

  // Delete a schedule
  deleteSchedule(id: string): Observable<ApiResponse<null>> {
    return this.http.delete<ApiResponse<null>>(`${this.apiUrl}/admin/schedules/${id}`);
  }

  // Cancel a schedule
  cancelSchedule(id: string): Observable<ApiResponse<BusSchedule>> {
    return this.http.patch<ApiResponse<BusSchedule>>(`${this.apiUrl}/admin/schedules/${id}/cancel`, {});
  }

  // Get schedules by date range
  getSchedulesByDateRange(startDate: string, endDate: string): Observable<ApiResponse<BusSchedule[]>> {
    return this.http.get<ApiResponse<BusSchedule[]>>(`${this.apiUrl}/admin/schedules/range`, {
      params: {
        startDate,
        endDate
      }
  ...
  getSchedulesByBus(busId: string): Observable<ApiResponse<BusSchedule[]>> {
    return this.http.get<ApiResponse<BusSchedule[]>>(`${this.apiUrl}/admin/schedules/bus/${busId}`);
  }

  // Get schedules by route
  getSchedulesByRoute(routeId: string): Observable<ApiResponse<BusSchedule[]>> {
    return this.http.get<ApiResponse<BusSchedule[]>>(`${this.apiUrl}/admin/schedules/route/${routeId}`);
  }

  // Check seat availability for a schedule
  checkSeatAvailability(scheduleId: string): Observable<ApiResponse<{ availableSeats: number; totalSeats: number }>> {
    return this.http.get<ApiResponse<{ availableSeats: number; totalSeats: number }>>(
      `${this.apiUrl}/admin/schedules/${scheduleId}/seats`
    );
  }
  ...
  getScheduleStatusOptions(): string[] {
    return ['scheduled', 'cancelled', 'completed', 'delayed'];
  }
}