...
export class MyBookingsComponent implements OnInit {
  ...
  calculateStatistics() {
    this.totalBookings = this.bookings.length;
    this.totalSpent = this.bookings.reduce((sum, b) => sum + (b.totalAmount || 0) + (b.taxAmount || 0), 0);
  ...
  applyFilters() {
    ...
    if (this.searchTerm.trim()) {
      ...
      filtered = filtered.filter(booking => {
        ...
        const busName = (booking.busDetails?.busName || '').toLowerCase();
        const origin = (booking.routeDetails?.origin || '').toLowerCase();
        const destination = (booking.routeDetails?.destination || '').toLowerCase();
        
        return bookingId.includes(term) || busName.includes(term) || 
               origin.includes(term) || destination.includes(term);
      });
    }
    ...
    if (this.selectedMonth !== 'all') {
      const month = parseInt(this.selectedMonth);
      filtered = filtered.filter(booking => {
        const bookingDate = new Date(booking.journeyDate || booking.travelDate || '');
        return bookingDate.getMonth() === month;
      });
    }
    ...
    // Sort by date (newest first)
    filtered.sort((a, b) => {
      const dateA = new Date(a.journeyDate || a.travelDate || a.createdAt || 0);
      const dateB = new Date(b.journeyDate || b.travelDate || b.createdAt || 0);
  ...
  getSeatNumbers(booking: Booking): string {
    ...
    return seats.map((s: any) => s.seatNumber).join(', ');
  }

  getPassengerName(booking: Booking): string {
    const seats = booking.seats || [];
    if (seats.length > 0) {
      return seats[0].passengerName || seats[0].name || 'N/A';
    }
    return 'N/A';
  }
  ...
  getFromLocation(booking: Booking): string {
    return booking.routeDetails?.origin || 'N/A';
  }

  getToLocation(booking: Booking): string {
    return booking.routeDetails?.destination || 'N/A';
  }

  getJourneyDate(booking: Booking): string {
    const date = booking.journeyDate || booking.travelDate || '';
    if (!date) return 'N/A';
    return new Date(date).toLocaleDateString('en-US', {
      day: 'numeric',
      month: 'short',
